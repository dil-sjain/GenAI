...
class LegacyTenantWorkflow
{
    /**
     * @var AxonIvyData Axon Ivy model
     */
    public $ivy;
    ...
    {
        $this->app = \Xtra::app();
        $this->ivy = (isset($ftr)) ? new AxonIvyData($ftr) : new AxonIvyData();
        $this->ivy->DB->clientDB = $this->ivy->DB->getClientDB($this->tenantID);
    ...
    {
        $featuresChecked = (new TenantFeatures($this->tenantID))->tenantHasFeatures(
            [\Feature::TENANT_API_INTERNAL, \Feature::TENANT_WORKFLOW],
            \Feature::APP_TPM
        );
        return ($featuresChecked[\Feature::TENANT_API_INTERNAL] && $featuresChecked[\Feature::TENANT_WORKFLOW]);
    }
    ...
    {
        $sql = "SELECT className FROM {$this->ivy->DB->globalDB}.g_workflowClass"
            . " WHERE tenantID = :tenantID";
        $bind = [
            ':tenantID' => $tenantID
        ];
        return $this->ivy->DB->fetchValue($sql, $bind);
    }
    ...
    {
        $sql = "SELECT COUNT(*) FROM {$this->ivy->DB->globalDB}.g_workflowEvent "
    ...
    public function autoSendDDQ($thirdParty, $formType = IntakeFormTypes::DUE_DILIGENCE_SBI)
    {
        if (!empty($thirdParty['POCemail'])) {
            ...
            $invitationControl = new InvitationControl(
                ...
                $this->ivy->app->ftr->has(FeatureACL::SEND_INVITE),
                $this->ivy->app->ftr->has(FeatureACL::RESEND_INVITE)
            );
    ...
    {
        // Get the profile id.
        $sql = "SELECT tpID FROM {$this->ivy->DB->clientDB}.cases WHERE id = :caseID";
        $profileID = $this->ivy->DB->fetchValue($sql, [':caseID' => (int)$caseID]);
        // See if Demo has the appropriate workflow event.
        if (!($profileID && $this->tenantHasEvent(self::DDQ_SUBMITTED))
            || !$this->ivy->getTaskForEvent($this->tenantID, $profileID, "profile", self::DDQ_SUBMITTED, false)
        ) {
            return;
        }
        // Update the Axon Ivy DDQ Submitted task.
        $this->ivy->sendEventSignal(
            $profileID,
            "profile",
            $this->tenantID,
            "complete",
        ...
        // Get the ddq field value - this should probably be placed into the internal API as an endpoint.
        $sql = "SELECT {$question} FROM {$this->ivy->DB->clientDB}.ddq WHERE id = :id";
        $answer = $this->ivy->DB->fetchValue($sql, [':id' => (int)$ddqID]);
        ...
        // Provide a delay for asynchronous calls if in the context of CLI.
        if ($delay) {
            sleep(10);
        }
        // Update workflow for DDQSubmitted.
        $this->ivy->sendRequest($this->tenantID, $fields, "/ThirdParty/ddqSubmitted", "POST", false);
    }
    ...
    {
        $wfTaskID = null;
        $result = $this->ivy->sendEventSignal(
            $profileID,
            'profile',
            $this->tenantID,
            $action,
            self::CASE_FOLDER_REVIEW,
            $determination
        );
        // Save note for task.
        if (is_array($result) && isset($result['data']['id'])) {
            $wfTaskID = $result['data']['id'];
            if ($wfTaskID) {
                $this->ivy->saveNote($this->tenantID, $wfTaskID, $explain);
            }
        }
    ...
    {
        $taskID = null;
        // Send signal to complete Profile Approval task.
        $result = $this->ivy->sendEventSignal(
            $profileID,
            'profile',
            $this->tenantID,
            $action,
            self::PROFILE_APPROVAL,
            $determination
        );
        // Save note for approval task, if the task already exists save over the previous note.
        if (is_array($result) && isset($result['data']['id'])) {
            $taskID = $taskID = $result['data']['id'];
        } else {
            $task = $this->ivy->getTaskForEvent(
                $this->tenantID,
                $profileID,
                "profile",
                self::PROFILE_APPROVAL
        ...
        if (isset($taskID)) {
            $this->ivy->saveNote($this->tenantID, $taskID, $explain);
        }
    ...
    {
        // @TODO: in the future passing in $recordType should differentiate between cases and profiles.
        $result = $this->ivy->sendRequest($tenantID, [], "/ThirdPartyProfile/{$recordID}/reviews/inProgress", "GET");
    ...
    {
        return $this->ivy->userCanLaunchBatchReview();
    }
    ...
    {
        return $this->ivy->reviewBatchLaunchAvailable($tenantID, $recordID, $recordType);
    }
    ...
    public function initialReviewBatchLaunch($tenantID, $recordID, $recordType = "profile")
    {
        ...
        return !$this->ivy->workflowExists($tenantID, $recordID);
    }
    ...
    {
        $sql = "SELECT rt.tierName AS risk FROM {$this->ivy->DB->clientDB}.thirdPartyProfile tp "
            . "LEFT JOIN {$this->ivy->DB->clientDB}.riskAssessment AS ra "
            . "ON (ra.tpID = tp.id AND ra.model = tp.riskModel AND ra.status = 'current') "
            . "LEFT JOIN {$this->ivy->DB->clientDB}.riskTier AS rt ON rt.id = ra.tier "
            . "LEFT JOIN {$this->ivy->DB->clientDB}.riskModelTier AS rmt "
            . "ON rmt.tier = ra.tier AND rmt.model = tp.riskModel "
            . "WHERE tp.id = :id";
        return $this->ivy->DB->fetchValue($sql, [':id' => $profileID]);
    }
    ...
    {
        $sql = "SELECT genQuestID, flaggedValue FROM {$this->ivy->DB->globalDB}.g_workflowFlaggedQuestions "
            . "WHERE tenantID = :tenantID";
        $questions = $this->ivy->DB->fetchAssocRows($sql, [':tenantID' => $this->tenantID]);
    ...
    public function checkFlaggedQuestions($ddqID)
    {
        if (count($this->flaggedQuestions) > 0) {
            ...
            $sql .= "FROM {$this->ivy->DB->clientDB}.ddq WHERE id = :id";
            $answers = $this->ivy->DB->fetchAssocRow($sql, [":id" => $ddqID]);
    ...
    public function getWorkflowNotificationDetails($iso)
    {
        ...
        if (!empty($iso)) {
            $sql = "SELECT w.*, u.userType, u.mgrDepartments, u.mgrRegions\n"
                . "FROM {$this->ivy->DB->globalDB}.g_workflowNotifications AS w\n"
                . "LEFT JOIN {$this->ivy->DB->authDB}.users AS u on u.id = w.userID\n"
                . "WHERE iso = :iso AND tenantID = :tenantID";
            $rtn = $this->ivy->DB->fetchAssocRow($sql, [':iso' => $iso, ':tenantID' => $this->tenantID]);
        }
        return $rtn;
    }
    ...
    {
        $required = [];
        $sql = "SELECT eventID FROM {$this->ivy->DB->globalDB}.g_workflowEvent "
            . "WHERE taskRequired = 1 AND tenantID = :tenantID";
        $events = $this->ivy->DB->fetchAssocRows($sql, [':tenantID' => $this->tenantID]);
