<?php
/**
 * Controller: Mubadala Tenant Custom Workflow
 */
...
/**
 * Delta copy of public_html/cms/includes/php/Models/TPM/Workflow/Mubadala.php provides Workflow functionality for
 * Mubadala (TenantID = 330)
 */
#[\AllowDynamicProperties]
class Mubadala extends TenantWorkflow
{
    /**
     * Mubadala constructor.
     *
    ...
    public function __construct($tenantID)
    {
        ...
        if (!$this->tenantHasWorkflow()
            || (!in_array($this->tenantID, [ClientIds::MUBADALA_CLIENTID]))
        ) {
            throw new \Exception("Not Mubadala or Mubadala doesn't have Workflow Feature/Setting enabled.");
        }
    }

    /**
     * Starts the Mubadala Third Party Profile approval workflow.
     *
    ...
    public function startProfileWorkflow($tpID, $caseID = null, $isUpload = false)
    {
        try {
            ...
            if (!empty($profile) && !$wfExists) {
                ...
                // Set the "conditional" property for the Mubadala workflow.
                $riskLevelMet = ($riskLabel == "high" || $riskLabel == "medium");
