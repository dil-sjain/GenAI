 ...
 *
 * @see Data class creates a PDF from a case folder and it's various sub-tabs
 */
...
class CasePdf
{
    ...
    private function getBizPracInfo($ddqRow, $ddqRowVals, $clientID, $langCode)
    {
        ...
        if ($ddqRow) {
            ...
            if ($isoCodes = $this->DB->fetchValueArray($sql, $params)) {
                if ($countryNames = (Geography::getVersionInstance())->getCountryNames($isoCodes)) {
     ...
     */
    private function getCaseNotesInfo($clientID, $caseID, $userClass)
    {
        $toPDF = true;
        $sortDir = 'DESC';
        $e_caseID = intval($caseID);
        ...
        if ($cnt) {
            ...
            $from = "FROM caseNote AS n "
                ...
                . "LEFT JOIN {$this->DB->authDB}.users AS u ON u.id = n.ownerID";
    ...
    public function getCasePdfInfo($authRow, $caseRow, $spDocs)
    {
        if (empty($authRow)
            ...
            return 'No object presented for case or auth.';
        }
        if (!isset($caseRow->caseType)
            || !isset($authRow->caseID)
            || !isset($authRow->clientID)
            || !isset($caseRow->tpID)
        ) {
            return "Properties for case or auth improperly present or missing.";
        }
        ...
        $tpID = $caseRow->tpID;
        if (!isset($caseTypeClientList[$caseRow->caseType])) {
            return 'Invalid value for caseType';
        }
        ...
        if ($tpFtrOn && $tpID) {
            ...
            if ($caseRow->rmID > 0 && $caseRow->raTstamp) {
                ...
                if (($recScope = $this->DB->fetchValue($sql, $params)) && ($recScope != $caseRow->caseType)) {
                    if (in_array($recScope, $caseTypeClientList)) {
        ...
        $sql = "SELECT langNameEng FROM {$this->DB->authDB}.languages WHERE langCode = :langCode";
        ...
        $sql = "SELECT userName, userEmail, userPhone FROM {$this->DB->authDB}.users WHERE id = :userID LIMIT 1";
        ...
        $sql = "SELECT userName FROM {$this->DB->authDB}.users WHERE id = :investigatorID";
        ...
        $e_requestor = $this->DB->esc($caseRow->requestor);
        $sql = "SELECT userName FROM {$this->DB->authDB}.users WHERE userid = :requestor LIMIT 1";
        $params = [':requestor' => $e_requestor];
        if (!($requestorName = $this->DB->fetchValue($sql, $params))) {
            $requestorName = $caseRow->requestor; // fall back to login id
        }

        $stateName
            = $this->geo->getStateNameTranslated($subjectInfoRow->country, $subjectInfoRow->state, $this->langCode);
        $countryName = $this->geo->getCountryNameTranslated($subjectInfoRow->country, $this->langCode);
        ...
        $pdfData = [
            ...
            'langName'      => $langName,
            'pdfTitle'      => 'Case.Folder.' . str_replace(" ", "_", (string) $caseRow->userCaseNum).'.pdf',
            'pageTitle'     => $pageTitle,
            'header'        => '',
            'footer'        => $footer,
            'logo'          => $logoFileName,
        ...
        $sql = "SELECT sia.id as dbid, description as fdesc, filename as fname, filename as fsrc, "
            . "fileType as ftype, dc.name as fcat, ownerID as owner, "
            . "fileSize as fsize, creationStamp as fdate, 0 as candel "
            . "FROM subInfoAttach as sia "
            . "LEFT JOIN docCategory as dc ON sia.catID = dc.id "
            . "WHERE caseID = :caseID AND sia.clientID = :clientID";
        $params = [':caseID' => $caseRow->id, ':clientID' => $clientID];
        $spDocs = $this->DB->fetchObjectRows($sql, $params);

        $pdfData['attachments'] = [];
        $pdfData['attachments']['spDocs']['docs']
            = $spDocs;//$this->getAttachmentsInfo($ddqRow, $clientID, $caseID, $spDocs);
        $pdfData['attachments']['spDocs']['total'] = count($spDocs);
        $sql = "SELECT * FROM ddq WHERE caseID = :caseID AND clientID = :clientID LIMIT 1";
        $params = [':caseID' => $caseRow->id, ':clientID' => $clientID];
        $ddqRow = $this->DB->fetchObjectRow($sql, $params);
        if (!$ddqRow) {
            $pdfData['attachments']['ddqAttachedMsg'] = false;
        } else {
            $pdfData['attachments']['ddqAttachedMsg'] = true;
            $sql = "SELECT bInfoQuestnrAttach FROM subjectInfoDD "
                . "WHERE caseID = :caseID AND clientID = :clientID LIMIT 1";
            $params = [':caseID' => $caseID, ':clientID' => $clientID];
            $attached = $this->DB->fetchValue($sql, $params);
            ...
            $pdfData['attachments']['ddqAttached'] = $attached;
        }
        ...
        if ($showNotes) {
            $pdfData['caseNotes'] = $this->getCaseNotesInfo($clientID, $caseID, $userClass);
        }
    ...
    private function getCompanyInfo(
        ...
        $ddqConstants = [IntakeFormTypes::DUE_DILIGENCE_SBI, IntakeFormTypes::DUE_DILIGENCE_SHORTFORM, IntakeFormTypes::DDQ_SHORTFORM_FORM2, IntakeFormTypes::DUE_DILIGENCE_SHORTFORM_RENEWAL, IntakeFormTypes::DDQ_SHORTFORM_FORM2_RENEWAL, IntakeFormTypes::DDQ_SHORTFORM_FORM3, IntakeFormTypes::DDQ_SHORTFORM_FORM3_RENEWAL, IntakeFormTypes::DDQ_SBI_FORM2, IntakeFormTypes::DDQ_SBI_FORM3, IntakeFormTypes::DDQ_SBI_FORM4, IntakeFormTypes::DUE_DILIGENCE_SBI_RENEWAL, IntakeFormTypes::DDQ_SBI_FORM2_RENEWAL, IntakeFormTypes::DDQ_SBI_FORM3_RENEWAL, IntakeFormTypes::DUE_DILIGENCE_SBI_RENEWAL_2, IntakeFormTypes::DDQ_SHORTFORM_4PAGE, IntakeFormTypes::DDQ_SHORTFORM_4PAGE_RENEWAL];
        $data = [];
        $iCompany = '';
        $iProfileRow = false;
        ...
        if (property_exists($authRow, 'iCompany')) {
            $iCompany = $authRow->iCompany;
        }
        if (property_exists($authRow, 'iName')) {
            $iName = $authRow->iName;
        }
        if (property_exists($authRow, 'iEmail')) {
            $iEmail = $authRow->iEmail;
        }

        // If the case was rejected, what was the rejection code
        $rejectCaseCode = '';
        $includeReason = in_array(
            $caseRow->caseStage,
            [CaseStage::CASE_CANCELED, CaseStage::CLOSED, CaseStage::CLOSED_HELD, CaseStage::CLOSED_INTERNAL]
        )) {
        ...
        if (!$ddqRow) {
            $data['countryOfRegistration'] = $this->geo->getCountryNameTranslated(
                $caseRow->caseCountry,
                $this->langCode
            );
        } else {
            ...
            if (property_exists((object)$aQuestionRow, 'labelText')) {
            ...
            if (in_array($ddqRow->caseType, $ddqConstants)) {
                ...
                // can't check session variables here - removed session and added $aOnlineQuestions
                foreach ($aOnlineQuestions as $i => $aOnlineQuestionsRow) {
                    // Find all PROFESSIONAL INFORMATION Section elements
                    if (0 && $aOnlineQuestionsRow->sectionName == "PROFESSIONAL INFORMATION"
                        && $data['profInfo']
                    ) {
            ...
            if (in_array($ddqRow->caseType, $ddqConstants)) {
                ...
                if ($aQuestionsRow) {
                    ...
                    $data['pocSection'] = $this->app->trans->codeKey('main_point_of_contact');
                }

                $mPOC = $this->app->trans->codeKey('main_point_of_contact');

                // Load MAIN POINT OF CONTACT Section
                foreach ($aOnlineQuestions as $i => $aOnlineQuestionsRow) {
                    // Find all of the MAIN POINT OF CONTACT Section elements
                    if ($aOnlineQuestionsRow->sectionName == $mPOC
                        || $aOnlineQuestionsRow->sectionName == "MAIN POINT OF CONTACT"
                    ) {
    ...
    private function getPersonalInfo(
        ...
        // Lets get the user defined investigation type name
        $ct =  ($subjectInfoRow->SBIonPrincipals == 'Yes')
            ? IntakeFormTypes::DUE_DILIGENCE_SBI : IntakeFormTypes::DUE_DILIGENCE_IBI;
        $prinCaseTypeName = $caseTypeClientList[$ct];
            ...
            if ($aOnlineQuestions && $ddqRow->caseType != IntakeFormTypes::DUE_DILIGENCE_SHORTFORM) {
                ...
                foreach ($aOnlineQuestions as $aOnlineQuestionsRow) {
                    ...
                    if ($aOnlineQuestionsRow->sectionName == 'KEY PERSON GENQUEST') {
                        $data['genericQuestions'][]['label'] = $this->ddq->getLabel($aOnlineQuestionsRow);
                        $data['genericQuestions'][]['value'] = $this->ddq->getVal(
                            $ddqRowVals,
                            $aOnlineQuestionsRow,
                            '50%'
                        );
    ...
    private function getRelationshipsInfo($ddqRow, $ddqRowVals, $clientID, $clientName, $langCode)
    {
        ...
        if ($aQuestionRow = $this->ddq->getQuestionFromPage($aOnlineQuestions, 'TEXT_GOVRELATION_SECTIONHEAD')) {
    ...
    private function getRiskRating($tpID, $clientID, $riskFtrOn)
    {
        ...
        $sql .= "FROM {$clientDB}.thirdPartyProfile AS tp "
            ...
            . "LEFT JOIN {$this->DB->authDB}.users AS own ON own.id = tp.ownerID "
            . "LEFT JOIN {$this->DB->authDB}.users AS orig ON orig.id = tp.createdBy ";
        if ($riskFtrOn) {
