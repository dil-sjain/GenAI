...
use Models\TPM\Admin\Utility\EmailFileManager;
use Lib\SettingACL;

/**
 * Class ManageDataFiles controller for managing admin file uploads
 */
class ManageEmailFiles extends Base
{
    use AjaxDispatcher;
    ...
    /**
     * Email File manager object
     *
     * @var object
     */
    protected $emailFileMgr = null;
     ...
     */
    protected $tpl = [
        'main' => 'emailFileManagement.tpl', // main page template
        'list' => 'emailFileManagementList.tpl' // file list tpl
    ];
    ...
    public function __construct($clientID, $initValues = [])
    {
        ...
        $this->emailFileMgr = new EmailFileManager($clientID);

        // Init local access to \Xtra::app() and \Xtra::app->request()
        $this->app     = \Xtra::app();
        $this->request = $this->app->request();

        $this->setViewValue('subDirOptions', json_encode(['System'=>'System', 'Client'=>'Client']));
    ...
    {
        $this->setViewValue('pgTitle', 'Upload/Manage Email Data Files');
        $this->setViewValue('previousEmailPg', '/adm/email');
    ...
    private function ajaxUploadDelete()
    {
        ...
        $msg      = '';
        try {
            $msg = $this->emailFileMgr->deleteFileByID($filename);
        } catch (\Exception $e) {
            $msg = $e->getMessage();
        }
        if (empty($msg)) {
        ...
        if ($validUpload !== true) {
            ...
            if ($upload && preg_match('/^[a-f0-9]{32}$/i', (string) $upload['fid'])) {
                if (!is_array($upload) || !file_exists($upload['uploadFile'])) {
                    ...
                    $result = $this->emailFileMgr->storeUploadedFile(
                        $upload['category'],
                        $upload['uploadFile'],
                        $upload['category'] . '_' . $upload['filename']
                    );
                }
            }
        }
    ...
    {
        $files = $this->emailFileMgr->getEmailFiles();
        // Get Smarty to lay it out
        $html = $this->app->view->render('TPM/Admin/Utility/emailFileManagementList.tpl', ['files' => $files]);
        if ($this->app->phpunit) {
