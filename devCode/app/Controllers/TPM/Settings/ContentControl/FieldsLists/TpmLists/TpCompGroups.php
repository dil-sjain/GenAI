<?php
/**
 * Control for TPM Fields/Lists - 3P Compliance Groups
 */
...
use Models\TPM\Settings\ContentControl\FieldsLists\TpmLists\TpCompGroupsData;

/**
 * Class TpCompGroups controls the 3P Compliance Group requirements. This class depends on being
 * called via the FieldsLists controller.
 *
 * @keywords 3P compliance groups, tpm, fields lists, content control, settings, custom lists
 */
#[\AllowDynamicProperties]
class TpCompGroups extends TpmLists
{
    /**
    ...
    public function __construct($tenantID, $listTypeID, $userID, $initValues = [])
    {
        ...
        $this->data = new TpCompGroupsData($this->tenantID, $this->userID, $this->listTypeID);
    ...
    {
        $this->jsObj->Result = 1;
        $this->jsObj->Items  = $this->data->getAll();
    }
