<?php
/**
 * Control for TPM Fields/Lists - 3P Type Categories
 */
...
use Models\TPM\Settings\ContentControl\FieldsLists\TpmLists\TpTypeCatsData;
use Models\TPM\Settings\ContentControl\FieldsLists\TpmLists\TpTypesData;

/**
 * Class TpTypeCats controls the 3P Type requirements. This class depends on being
 * called via the FieldsLists controller.
 *
 * @keywords 3P types, tpm, fields lists, content control, settings, custom lists
 */
#[\AllowDynamicProperties]
class TpTypeCats extends TpmLists
{
    /**
    ...
    public function __construct($tenantID, $listTypeID, $userID, $initValues = [])
    {
        ...
        $this->data = new TpTypeCatsData($this->tenantID, $this->userID, $this->listTypeID);
    ...
    {
        $this->jsObj->Result = 1;
        $types = new TpTypesData($this->tenantID, $this->userID, $this->listTypeID);
        $this->jsObj->Lists = $types->getKeyValList();
    }

    /**
     * Fetch records of specified parent for return to view
     *
     * @param array $post Sanitized post array (app()->clean_POST)
     *
     * @return void
    ...
    {
        $this->jsObj->Result = 1;
        $items = $this->data->getAllByGroup($post['tpTypeID']);
        if (!$items) {
            $this->jsObj->NoRows = 1;
